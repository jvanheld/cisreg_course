...
6. Download the matrix of the transcription factor you chose, and store it in a text file. 

7. In the **matrix tools** section of  [RSAT](http://teaching.rsat.eu/), open the **convert-matrix** form. Paste the matrix downloaded from footprintDB, and select the appropriate input format (note that the *footprintDB* format is a slight variation on the *transfac* format).

8. Compute the counts, frequencies, weights, consensus, parameters and logo for your matrix of interest, using an organism-specific background model estimated on all the non-coding upstream sequences from *Escherichia coli K12 mg1655*.
