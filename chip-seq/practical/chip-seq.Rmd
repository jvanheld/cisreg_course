---
title: "ChIP-Seq -- Quality filtering and read mapping --"
...
public repository that provide tools to submit, access and mine
...
> - In the upper left corner, click on **Unnamed history** and rename this workspace to 'ER\_chr21\_mapping'.
> - In the left menu, select **Get Data &gt; Upload File**. set File format as **fastqsanger**, copy and paste the URL above in the **URL/text** area and set genome to **hg19**.
> - Select **NGS TOOLS &gt; NGS: QC and manipulation &gt; FASTQ Groomer**.
> - Set **File to groom** to 'siNT\_ER\_E2\_r3.SRR540192.chr21.andNoise.fastq', and press **Execute**
> - Rename the output of the previous step into 'siNT\_ER\_E2\_r3.chr21.groom' (use the pencil that is associated to each item of the history to edit their attributes).
...
> - Select 'siNT\_ER\_E2\_r3.chr21.groom' in **Short read data from your current history** dropdown list. Press **execute**.
> - Display the data for the corresponding result in your **history**
    (right panel).
...
> -   From **reads fastq file** dropdown list select'siNT\_ER\_E2\_r3.SRR540192.chr21.andNoise.groom'. Set **Quality Threshold** to 20, **Length Threshold** to 25, **min\_len** to 25 and press **execute**.
> -   Rename the output into siNT\_ER\_E2\_r3.chr21.sickle
...
> - From the tool panel, select **NGS: SAM Tools &gt; rmdup remove PCR duplicates**. 
...
> - Download the **.bam** and **.bai** files produced by bowtie.
> - Download the **.bam** and **.bai** files produced by rmDup.
...
> - Do you observe some **duplicate reads** in the non-filtered bam ?
> - In order to produce a coverage file (**tdf** format), from IGV select **Tools > Run Igvtools**. 
...
**NB:**The tdf file is a IGV specific format that is closed to the
bigWig format (the compressed version of wig format).
...
**From the bowtie manual:** "Aligners characterize their degree of confidence in the point of origin by reporting a mapping quality: a non-negative integer Q = -10 log10 p, where p is an estimate of the probability that the alignment does not correspond to the read's true point of origin. Mapping quality is sometimes abbreviated MAPQ, and is recorded in the SAM MAPQ field". This information is available in the 5th column a a SAM file.

<div class="protocol">

> - Select **NGS: SAM Tools > Filter BAM or SAM on flags, fields, and tags using Sambamba** from the toolbox.
> - Filter the BAM filtered on duplicates based on mapping quality (30).
> - Use flagstats to check the number of read left. 
> - Check that the overall mean of MAPQ is improved after filtering using **NGS: SAM Tools > sam-stats**
...
Using the same procedure, to align reads obtained from the input sample. 

Input reads can be obtained [here](http://denis.puthier.perso.luminy.univ-amu.fr/COURSES/CHIP-SEQ/PRACTICAL/data/MCF_input_r3_SRX176888_chr21_Noise.fastq.gz).

**NB:** galaxy allow one to extract and run workflow. If you feel very comfortable with galaxy, you can use this solution. Alternatively, just run the same steps as the previous analysis.

<div class="protocol">

> - load the results for input into IGV. 
...
The R SPP package can be used to compute strand cross-correlation, one of the first criteria proposed to check the quality of a ChIP result.
...
Wiggle files can be used to display coverage results over a genome. Bigwig files are compressed and indexed version of the [wiggle (*.wig) file](http://genome.ucsc.edu/goldenpath/help/wiggle.html). The bigWig format is the recommended format for almost all graphing track needs (wiggle is an older format). 


We will create two bigWig, one for the ChIP and one for the input. We will next produce a combined file in which the background noise has 
...
>     - Effective genome size: use chromosome 21 size (48129895).
...
> - Order the peaks by length. Have a look at the broader peaks.
> - What does the summit.bed file contain ?
> - What does the peaks.xls and negative_peaks.xls files contains ?
> - Have a look at the set of negative peaks in IGV. What can we say ?
> - Using Galaxy tools. Define the TSS regions for each genes. Compute the closest TSS of each peak.
